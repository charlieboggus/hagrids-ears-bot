...
export class Logger {
    public static log(message: string, notify: boolean = false): void {
        ...
        if (notify) {
            snsClient.publishNotification(message, `[${new Date(Date.now()).toString()}][LOG] Hagrid Has something to say`)
        }
    }
    ...
    public static error(message: string, notify: boolean = true): void {
        ...
        if (notify) {
            snsClient.publishNotification(message, `[${new Date(Date.now()).toString()}][ERROR] Hagrid has gone deaf!`)
        }
    }
}